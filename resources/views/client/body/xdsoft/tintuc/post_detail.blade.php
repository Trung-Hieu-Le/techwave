                        ...
                        <div class="col-12 col-xl-4 col-md-6 mb-5 p-3 pt-0">
                            <a href="{{route('tintuc.post',['slug'=>$item->slug])}}" class="text-decoration-none">
                                <div class="card card-hover w-100 p-0 container border-0 text-start">
                                    <img src="{{$item->post_image}}" class="rounded mx-auto d-block img-fluid" style="width:250px; height:180px;" alt="...">
                                    <div class="card-body mb-3">
                                        <h5 class="card-title overflow-hidden text-black">{{$item->post_title}}</h5>
                                        <h6 class="card-title overflow-hidden text-dark text-decrip-2" style="height: 40px">{{$item->description}}</h6>
                                    </div>
                                </div>
                            </a>
                        </div>
        ...
        <div id="post-comment" class="mt-4">
            ...
            <form action="{{ route('tintuc.addComment') }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <textarea name="content" id="content" class="form-control" style="background-color: white!important; color:black!important;"
                    placeholder="Nhập bình luận của bạn" rows="4" required></textarea>
                </div>
            ...
            <div class="comment-box mb-4">
                ...
                <div class="">
                    ...
                    <div class="comment-actions text-right mb-1" style="text-align: right;">
                        <button class="btn btn-primary btn-sm" onclick="toggleReplyForm({{ $comment->id }})">Trả lời</button>
                        @if($comment->id_user != session('account_id'))
                        <button class="btn btn-danger btn-sm" onclick="reportComment({{ $comment->id }})">Báo cáo</button>
                        @endif
                    </div>
                    ...
                    <div id="reply-form-{{ $comment->id }}" class="reply-form mt-2" style="display: none;">
                        <form action="{{ route('tintuc.replyComment') }}" method="POST">
                            ...
                            <input type="hidden" name="id_user" value="{{ session('account_id') }}">
                            <textarea name="content" class="form-control mb-2" rows="2" style="background-color: white!important; color:black!important;"
                    ...
                    <div class="replies mt-3" style="margin-left: 50px;">
                        ...
                        <div class="reply-box ml-3 mb-2">
                            ...
                            <div class="">
                                ...
                                <div class="comment-actions text-right mb-1" style="text-align: right;">
                                    <button class="btn btn-primary btn-sm" onclick="toggleReplyForm({{ $reply->id }})">Trả lời</button>
                                    @if($reply->id_user != session('account_id'))
                                    <button class="btn btn-danger btn-sm" onclick="reportComment({{ $reply->id }})">Báo cáo</button>
                                    @endif
                                </div>
                                ...
                                <div id="reply-form-{{ $reply->id }}" class="reply-form mt-2" style="display: none;">
                                    <form action="{{ route('tintuc.replyComment') }}" method="POST">
                                        ...
                                        <textarea name="content" class="form-control mb-2" rows="2" style="background-color: white!important; color:black!important;"
                                        placeholder="Nhập phản hồi của bạn" required></textarea>
                                        <button type="submit" class="btn btn-primary btn-sm mb-2">Gửi phản hồi</button>
                                    </form>
                                </div>
                            </div>
                        </div>
