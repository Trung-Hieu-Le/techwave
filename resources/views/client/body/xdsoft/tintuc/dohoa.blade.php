...
@section('css')
  ...
  .menu-tintuc li a.active, .menu-tintuc li a:hover{
    ...
    .phan_trang ul.pagination{font-size: 1.4rem; margin: 10px 0 20px 0 !important; padding-left: 10px !important;}
      ...
      <div class="row top_1">
        <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
          <a href="{{route('tintuc.post',['slug'=>$post[0]->slug])}}">
            <img {{-- data-src="https://laptopxaydung.com/upload/assets/large/2023/0707/ne/01/i9/29-2023-07-07-3-17-53.png" --}}
              src="{{$post[0]->post_image}}" alt="{{$post[0]->post_title}}"
              class="img-responsive img_1 lazy" style="height: 350px; width:650px; object-fit:cover;">
            </a>
        </div>
        <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
          <a href="{{route('tintuc.post',['slug'=>$post[0]->slug])}}" title="">
            <p class="text-uppercase py-4 text-dark " style="font-size: 22px; font-weight:bold;">{{$post[0]->post_title}}</p>
          </a>
        </div>
      </div>
      <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
                ...
                <div class="py-2">
                  <div class="row">
                    <div class="col-xs-12 col-sm-12 col-md-5 col-lg-55">
                      <a class="thumnail-baiviet" href="{{route('tintuc.post',['slug'=>$item->slug])}}">
                        <img class="mr-3 img-fluid"
                          src="{{$item->post_image}}" alt="{{$item->post_title}}" style="height: 180px; width:240px; object-fit:cover;">
                      </a>
                    </div>
                    <div class="col-xs-12 col-sm-12 col-md-7 col-lg-7">
                      <div class="media-body">
                        <div class="title-baiviet"><a style="font-size: 18px; font-weight:600;"
                            href="{{route('tintuc.post',['slug'=>$item->slug])}}">{{$item->post_title}}</a></div>
                        <div class="description-baiviet" style="font-size: 14px; margin: 10px 0 10px 0;">{{$item->description}}</div>
                        <div class="info-baiviet"><span class="date-create" style=" color: #9f9f9f; font-size: 12px;">{{date("d/m/Y", strtotime($item->post_date))}}</span></div>
                      </div>
                    </div>
                  </div>
                </div>
      ...
      <div class="top_2">
        <div class="top_2_5">
          <div class="row block_1">
            <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 ">
              <p class="fw-bold m-0 fs-3" style="color:rgb(0, 170, 255);border-bottom: 1px solid #dcdcdc ;">Tin nổi bật
              </p>
            </div>
          </div>
          <div class="row p-3">
                    ...
                    <div class="d-flex my-2" style="padding-left: 0; padding-right:0;">
                      <div style="width:30%">
                        <img src="{{$item->post_image}}"
                          alt="{{$item->slug}}" style="height: 70px; width:100px; object-fit:cover;">
                      </div>
                      <div style="width:70%">
                        <a href="{{route('tintuc.post',['slug'=>$item->slug])}}"
                          title="{{$item->slug}}">
                          <p style="font-size: 14px; margin-left:10px; font-weight:bold; color:black;">{{$item->post_title}}</p>
                        </a>
                      </div>
                    </div>
